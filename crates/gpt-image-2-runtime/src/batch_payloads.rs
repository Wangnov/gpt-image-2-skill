use gpt_image_2_core::delete_history_job;
...
impl BatchItemError {
    pub fn from_error_value(index: usize, error: Value) -> Self {
        ...
        let detail = error.get("detail").cloned();
        Self {
            index,
            code,
...
pub fn collect_history_ids(payload: &Value) -> Vec<String> {
    let mut ids = Vec::new();
...
pub fn output_files_from_payload(payload: &Value) -> Vec<Value> {
...
pub fn normalize_batch_output(files: Vec<Value>) -> Value {
...
pub fn batch_errors_json(errors: &[BatchItemError]) -> Value {
    Value::Array(
        errors
            .iter()
            .map(|error| {
...
pub fn batch_error_summary(errors: &[BatchItemError]) -> Option<String> {
    if errors.is_empty() {
        return None;
    }
    let first = errors
...
pub fn cleanup_child_history(payload: &Value, app_job_id: &str) {
...
mod tests {
    use super::*;
    use crate::{job_from_payload, terminal_event_type, terminal_status_runs_storage_upload};
