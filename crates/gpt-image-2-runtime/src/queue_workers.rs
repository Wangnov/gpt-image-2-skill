...
pub fn append_terminal_queue_event(
    state: &JobQueueState,
    job_id: &str,
    event_type: &str,
    event_data: Value,
...
pub fn upload_completed_job_outputs(job: &Value) -> Result<Value, String> {
    let _ = persist_job(job);
    let job_id = job
        .get("id")
        .and_then(Value::as_str)
...
pub fn spawn_storage_upload_then_notify(state: JobQueueState, job_id: String, job: Value) {
    thread::spawn(move || {
...
pub fn spawn_notification_dispatch(state: JobQueueState, job_id: String, job: Value) {
    thread::spawn(move || {
...
pub fn start_queued_jobs(state: JobQueueState) {
    loop {
...
pub fn enqueue_job(state: JobQueueState, queued: QueuedJob) -> Result<Value, String> {
