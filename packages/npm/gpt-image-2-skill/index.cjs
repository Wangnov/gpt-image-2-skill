...
function probeBinary(binaryPath) {
  const result = childProcess.spawnSync(binaryPath, ["--json", "config", "path"], {
    encoding: "utf8",
  ...
  if (result.status !== 0) {
    return { ok: false, message: `${binaryPath} --json config path failed with status ${result.status}`, stderr: result.stderr || "" };
  }
