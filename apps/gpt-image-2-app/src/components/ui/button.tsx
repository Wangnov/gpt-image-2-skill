...
const button = cva(
  "inline-flex items-center gap-1.5 font-medium leading-none whitespace-nowrap border rounded-md border-transparent transition-[background,border-color,color] duration-100 select-none disabled:opacity-45 disabled:cursor-not-allowed",
  {
    variants: {
      variant: {
        primary:
          "bg-accent text-[color:var(--accent-on)] border-[color:var(--accent)] hover:bg-accent-hover",
        secondary: "bg-raised text-foreground border-border hover:bg-hover",
        ghost: "bg-transparent text-foreground hover:bg-hover",
        danger:
          "bg-raised text-status-err border-border hover:bg-status-err-bg",
        solidDark:
          "bg-[color:var(--n-900)] text-[color:var(--n-0)] border-[color:var(--n-900)] hover:bg-[color:var(--n-700)]",
      },
      size: {
    ...
    compoundVariants: [{ variant: "ghost", active: true, class: "bg-pressed" }],
